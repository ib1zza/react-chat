...
export const selectChat = (user: IUserInfo) => (dispatch: AppDispatch, getState: () => RootState) => {
    const currentUserId = getState().user.authUser?.uid;
    if (!currentUserId) return;

    dispatch(changeUser({chatId:  currentUserId > user.uid
            ? currentUserId + user.uid
            : user.uid + currentUserId, user: user}))
}
...
export const {selectChatId, selectChatUser, selectChatData} = chatSlice.selectors
